				...
				foreach($userSubscriptions as $subscription){
					try {
						...
						if(!empty($subscrObj)){
							...
							$subStart = $subscrObj['start'];
							$current_period_start = $subscrObj['current_period_start'];

							$now = time();
							$datediff = $now - $current_period_start;
							$Curdifernce =  round($datediff / (60 * 60 * 24));
                            ...
                            elseif($Curdifernce > $plan_time){
								//no charged after duration
								$isCharged = false;
								$isValid = false;
							...
							if( (!empty($isCharged) || !empty($isActive)) && !empty($isValid) ){
								//subscription is valid and make listing published
								...
								//subscription is invalid and make listing expired
